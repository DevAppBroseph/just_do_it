...
class MoveTaskToTopButton extends StatelessWidget {
  const MoveTaskToTopButton(
      {super.key, required this.task, required this.user, required this.updateTask});
               ...
               onTap: () async {
                 final isOwner=task.owner?.id == user?.id;
                   if (context.mounted) {
                     ...
                     final isUpgradeSuccessful = await Repository().editTask(
                         accessToken, newTask);
                     if (isUpgradeSuccessful) {
                       if (context.mounted) {
                         ...
                         if(isOwner){
                           ...
                           onTopDialog(context,'up_to'.tr(),
                               'response_is_fixed_in_the_top'.tr(), 'your_ad_is_now_above_others'.tr());
                         }
